...
/- Unexecuted C08 companion source; verdict: candidate_only.
   The quoted import names the C08 source artifact, not an installed package.
   An authorized replay must first build that exact source to its matching
   module object and set the candidate-source directory as a module path.
   No compilation or mathematical receipt is reported.

   Main scope: acyclic induced colorings on all proper vertex subsets imply
   strong connectivity of a non-colorable relation; with asymmetry they also
   imply four distinct neighbors at every vertex. Finite planar vertex-count
   minimality implies the proper-subset premise via a separate semantic bridge. -/
